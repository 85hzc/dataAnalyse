...
id_list = []
info_list = []
for i in range(0, 1000, 100):    # 已改成0-20来方便测试，改回9970便可爬去2018年全部电影
    parse_index_page(
        "https://movie.douban.com/j/new_search_subjects?sort=U&range=0,10&tags=%E7%94%B5%E5%BD%B1&start=" + str(
            i) + "&year_range=2018,2019")
    print('spider_film parse_index_page %d'%i)
...
for temp in id_list:
    ...
    j = j + 1
    print('spider_film parse film detail %d'%j)
    time.sleep(1)
